...
def initialize_profile(profile):
    ...
    create_schedule(profile=profile, price=price)


def get_schedule(profile=None, customer=None):
    if profile:
        customer, _ = djstripe_models.Customer.get_or_create(profile)
    if customer is None:
        raise UserOrCustomerNotDefined("Either profile or customer must be defined")

    return customer.schedules.filter(status=djstripe_enums.SubscriptionScheduleStatus.active).first()


def create_schedule(
    subscription: djstripe_models.Subscription = None, price: djstripe_models.Price = None, profile=None, customer=None
):
